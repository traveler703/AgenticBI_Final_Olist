...
项目仓库链接：https://github.com/yu2486789817/AgenticBI_Final_Olist/tree/main
...
| `mv_delivery_perf` | 年月、州 | 订单级配送时长、准时率与延迟诊断 |
...
跨期/跨组聚合率与均值时强制加权：`SUM(rate * cnt) / SUM(cnt)`，而非对各分组率求简单平均，避免小样本拉偏（如准时率）。该约束写入数据字典提示与反思质检。底层预聚合先统一到订单粒度，再进行跨月份或跨州的样本量加权，避免多商品订单获得额外权重。
...
| 马小龙 | 查询驱动可视化、前端交互界面、系统测试、项目报告与演示材料   | 25%      |
